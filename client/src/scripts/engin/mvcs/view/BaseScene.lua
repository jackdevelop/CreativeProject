...
function BaseScene:ctor(param)
    ...
    self.bgSprite_ = display.newSprite(imageName)
    ...
    if width then self.bgSprite_:setContentSize(CCSize(width,height)); end
	local contentSize_=self.bgSprite_:getContentSize();
	self.width_=contentSize_.width;
	self.height_=contentSize_.height;
    ...
    self.flysLayer_ = display.newNode()
...
--    if device.platform == "android" then -- 安桌平台,返回特殊处理代码
--        self.keypadLayer = display.newLayer()
--        self.keypadLayer:addKeypadEventListener(function(event)
--            if event == "back" then -- 按了返回键
--                local sceneName = self.__cname
--                local target = self._sceneToggle[sceneName]
--                if not target or target == "quit" then -- 直接退出
--                    CCDirector:sharedDirector():endToLua()
--                elseif target == "confirm-quit" then -- 确认退出
--                    local function quitCallback(alertMark)
--                        if alertMark == BaseUI.OK_BTN then -- OK
--                            CCDirector:sharedDirector():endToLua()
--                        else -- Cancel
--                            -- do nothing
--                        end
--                    end
--                    local param = {
--                        type = AlertUI.TS , --AlertUI.ZY  --AlertUI.SM
--                        callBack = quitCallback, --handle(self,self.handleAlert),
--                        text = Language.getValueByKey(1259),
--                        buttonCount = 2, --指定两个按钮 （即有确定 取消，默认只有一个确定按钮）
--                    }
--                    AlertUI.new(param);
--                else -- 切换场景 
--                	GameUtil.replaceScene(target)
--                end
--            end
--        end)
--        self:addChild(self.keypadLayer)
--        self.keypadLayer:setKeypadEnabled(true)
--    end
end
...
function BaseScene:isViewCreated()
...
function BaseScene:onTouch(event, x, y)
	...
	elseif event == "moved" then
		if self.drag then
			...
			if not self.touchBuildView then
...
function BaseScene:multiTouchHandle( event,points)
    ...
    elseif event == "moved" then
        if self.pointArr_[0] and self.pointArr_[1] then -- 缩放时只取前两个触点
            local p1, p2 = self.pointArr_[0], self.pointArr_[1]
            local dist = Math2d.dist(p1.x, p1.y, p2.x, p2.y) -- 两触点间的距离
...
function BaseScene:onEnter()
	if self.touchLayer_ then 
	 	self.touchLayer_:addTouchEventListener(function(event, x, y)
		 	if self.multiTouch_ then --多点触摸处理
		 		return self:multiTouchHandle( event,x)-- points 是包含一系列坐标的表格对象，内容为 {x, y, [x, y], [x, y] ...}
		 	else
		 		 return self:onTouch(event, x, y)
		 	end
	 	
	    end,self.multiTouch_,SceneConstants.LayerEventPriorityConstants_touchLayer,false)
	    self.touchLayer_:setTouchEnabled(true)
	    
	    if self.multiTouch_ then
